...
const UsersList = ({ users, deleteUser, handleUpdateUser }) => {
  // console.log(users);
  return (
    <section className="w-screen h-full grid gap-4 my-10">
      {users.map((user) => (
        <article
          key={user.id}
          className="w-[90vw] mx-auto rounded-lg bg-light-gray-2 grid grid-cols-4 p-4 gap-2"
        >
          ...
          <div className="flex flex-row justify-between items-start">
            <button onClick={() => deleteUser(user.id)} className="bg-red-500 p-1 rounded-md">
              <IconTrash />
            </button>
